...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === "reset-password") { if (!body.password || body.password.length < 8) return Response.json({ error: "新密码至少 8 位。" }, { status: 400 }); if (!isAsciiPassword(body.password)) return Response.json({ error: "新密码只能使用半角英文、数字和符号。" }, { status: 400 }); await auth.api.setUserPassword({ body: { userId, newPassword: body.password }, headers: request.headers }); await auth.api.revokeUserSessions({ body: { userId }, headers: request.headers }); }
