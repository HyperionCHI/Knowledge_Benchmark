...
import { grantsFromLegacyScopes, readWorkspaceUserPermissions, replaceWorkspaceUserPermissions } from "../../../db/workspace-permissions";
...
export async function GET(request: Request) {
  ...
  return Response.json({ users: rows.results.map((row) => ({
    ...
    ...(readWorkspaceUserPermissions(state, state.users.find((profile) => profile.id === row.id) || { id: row.id, name: row.name, email: row.email, role: row.role, scopes: [] })),
  })) });
}
...
export async function POST(request: Request) {
  const access = await requireAdmin(request); if (access.response) return access.response;
  try {
    const body = await request.json() as { username?: string; password?: string; name?: string; role?: WorkspaceUser["role"]; scopes?: string[]; generalPermission?: "view" | "edit"; grants?: WorkspaceUser["grants"] };
    ...
    if (role !== "admin") {
      ...
      replaceWorkspaceUserPermissions(state, profile.id, body.generalPermission || legacyPermission, grants, access.session.user.id);
    }
    ensureWorkspaceRecordSchema(state); audit({ id: access.session.user.id, name: access.session.user.name }, "create", "user", profile.id, `${name} · ${role === "admin" ? "管理员" : "普通成员"}`);
    return Response.json({ user: { ...profile, ...readWorkspaceUserPermissions(state, profile), username: usernameValue } }, { status: 201 });
  } catch (error) {
