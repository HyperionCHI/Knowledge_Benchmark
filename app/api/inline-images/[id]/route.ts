...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (row.scope === "sop" && (!access.state || !canViewWorkspaceScope(access.state, access.profile, row.brand, row.product))) return Response.json({ error: "没有该正文图片的访问权限。" }, { status: 403 });
