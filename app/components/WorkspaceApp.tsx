...
import { ASCII_PASSWORD_PATTERN, USERNAME_PATTERN, isAsciiPassword, sanitizeAsciiPassword, sanitizeUsername } from "../lib/credential-policy";
...
import { canEditGeneralContent, canEditWorkspaceScope, resolveWorkspacePermission } from "../lib/workspace-permissions";
...
type ScopeGrant = { scopeType: "all" | "brand" | "product"; scopeId: string; permission: "none" | "view" | "edit" };
...
type UserRecord = { id: string; email: string; name: string; role: Role; scopes?: string[]; generalPermission?: "view" | "edit"; grants?: ScopeGrant[]; banned?: boolean };
...
function scopeCanEdit(profile: UserRecord, data: WorkspaceState, brand: string, product: string) { return canEditWorkspaceScope(data, { ...profile, scopes: profile.scopes || [] }, brand, product); }
...
function WorkspaceCore({ sessionUser }: { sessionUser: { id: string; name: string; email: string; role?: string } }) {
  ...
  const canEdit = canEditGeneralContent({ ...profile, scopes: profile.scopes || [] });
    ...
    <main className="workspace-app">
      ...
      {loadError ? <div className="page-shell state-page"><ActionIcon name="refresh" size={26} /><h1>工作台加载失败</h1><p>{loadError}</p><button className="primary" onClick={loadWorkspace}>重新加载</button></div> : noPermission ? <div className="page-shell state-page"><ActionIcon name="admin" size={26} /><h1>无权限访问</h1><p>当前账号没有访问该品牌或产品线的权限。</p><button className="primary" onClick={() => { setNoPermission(false); navigate("home"); }}>返回工作台</button></div> : view === "home" ? <Home onNavigate={navigate} onSearch={() => setSearchOpen(true)} /> : (
        <div className="page-shell">
          <Breadcrumb view={view} brand={brand} product={product} title={titleMap[view] || ""} navigate={navigate} actions={view === "docs" && canEdit ? <button className="primary breadcrumb-action" onClick={() => setDocsStructureOpen(true)}><ActionIcon name="edit" />编辑层级</button> : view === "other-docs" && canEdit ? <button className="primary breadcrumb-action" onClick={() => setOtherDocsStructureOpen(true)}><ActionIcon name="edit" />编辑层级</button> : view === "sop-detail" && scopeCanEdit(profile, data, brand, product) ? <button className="primary breadcrumb-action" onClick={() => setSopStructureOpen(true)}><ActionIcon name="edit" />编辑层级</button> : null} />
          {view === "docs" && <DocsPage draftOwnerId={profile.id} data={data} selected={selectedDoc} onSelect={selectDoc} canEdit={canEdit} canManageStructure={profile.role === "admin"} structureOpen={docsStructureOpen} onCloseStructure={() => setDocsStructureOpen(false)} setData={setData} onCategoryAction={manageDocCategory} toast={toast} />}
          {view === "sop-brands" && <BrandPicker title="品牌 / 产品 SOP" description="先选择品牌，再进入对应产品线的 SOP。" brands={data.brands} icons={data.brandIcons} canSort={profile.role === "admin"} onReorder={(names) => reorderCatalog("brands", names)} onPick={(value) => navigate("sop-products", value)} />}
          {view === "sop-products" && <ProductPicker title="品牌 / 产品 SOP" brand={brand} products={data.productsByBrand[brand] || data.products} icons={data.productIcons} canSort={profile.role === "admin"} onReorder={(names) => reorderCatalog("products", names, brand)} onPick={(value) => navigate("sop-detail", brand, value)} />}
          {view === "sop-detail" && <SopPage draftOwnerId={profile.id} key={data.productIds[`${brand}/${product}`]} data={data} brand={brand} product={product} selected={selectedSopDoc} canEdit={scopeCanEdit(profile, data, brand, product)} canManageCollaborators={profile.role === "admin"} structureOpen={sopStructureOpen} onCloseStructure={() => setSopStructureOpen(false)} setData={setData} onSelect={(nextDoc) => navigate("sop-detail", brand, product, nextDoc)} toast={toast} />}
          {view === "tracker-brands" && <BrandPicker title="项目跟踪表格索引" description="按品牌与产品线集中管理项目跟踪入口。" brands={data.brands} icons={data.brandIcons} canSort={profile.role === "admin"} onReorder={(names) => reorderCatalog("brands", names)} onPick={(value) => navigate("tracker-products", value)} />}
          {view === "tracker-products" && <ProductPicker title="项目跟踪表格索引" brand={brand} products={data.productsByBrand[brand] || data.products} icons={data.productIcons} canSort={profile.role === "admin"} onReorder={(names) => reorderCatalog("products", names, brand)} onPick={(value) => navigate("tracker-detail", brand, value)} />}
          {view === "tracker-detail" && <TrackerPage data={data} brand={brand} product={product} canEdit={scopeCanEdit(profile, data, brand, product)} setData={setData} toast={toast} />}
          {view === "other-docs" && <DocsPage draftOwnerId={profile.id} kind="other" data={data} selected={selectedOtherDoc} onSelect={selectOtherDoc} canEdit={canEdit} canManageStructure={profile.role === "admin"} structureOpen={otherDocsStructureOpen} onCloseStructure={() => setOtherDocsStructureOpen(false)} setData={setData} onCategoryAction={manageOtherDocCategory} toast={toast} />}
...
function AuthScreen({ onAuthenticated }: { onAuthenticated: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(""); const form = new FormData(event.currentTarget); const username = String(form.get("username") || ""), password = String(form.get("password") || "");
    if (!new RegExp(`^${USERNAME_PATTERN}$`).test(username)) return setError("账号需为 3–30 位字母、数字、点、下划线或短横线。");
    if (!isAsciiPassword(password)) return setError("密码只能使用半角英文、数字和符号。");
    setBusy(true);
    try {
      if (needsSetup) {
...
function DocsPage({ draftOwnerId, kind = "general", data, selected, onSelect, canEdit, canManageStructure, structureOpen, onCloseStructure, setData, onCategoryAction, toast }: { draftOwnerId: string; kind?: "general" | "other"; data: WorkspaceState; selected: string; onSelect: (id: string) => void; canEdit: boolean; canManageStructure: boolean; structureOpen: boolean; onCloseStructure: () => void; setData: (next: WorkspaceState) => void; onCategoryAction: (action: "create" | "rename" | "delete", name: string, newName?: string, options?: { categoryId?: string; parentId?: string | null }) => Promise<boolean>; toast: (message: string) => void }) {
  ...
  return <section className="docs-page">
    ...
    <div className={`docs-layout ${navCollapsed ? "collapsed" : ""}`}><aside title="资料目录"><button className="docs-nav-toggle" type="button" aria-expanded={!navCollapsed} aria-label={navCollapsed ? "展开目录" : "收起目录"} title={navCollapsed ? "展开目录" : "收起目录"} onClick={() => setNavCollapsed((current) => !current)}><ActionIcon name={navCollapsed ? "expandPanel" : "collapsePanel"} /><span>{navCollapsed ? "展开目录" : "收起目录"}</span></button>{!navCollapsed && topCategories.map((category) => navigationCategory(category))}</aside>
...
function SopPage({ draftOwnerId, data, brand, product, selected, canEdit, canManageCollaborators, structureOpen, onCloseStructure, setData, onSelect, toast }: { draftOwnerId: string; data: WorkspaceState; brand: string; product: string; selected: string; canEdit: boolean; canManageCollaborators: boolean; structureOpen: boolean; onCloseStructure: () => void; setData: (next: WorkspaceState) => void; onSelect: (documentId?: string) => void; toast: (message: string) => void }) {
  ...
  function navigationCategory(category: SopCategoryRecord, level = 0): ReactNode {
    const categoryDocs = documentsIn(category), children = childrenOf(category), collapsed = collapsedCategories.has(category.id), total = categoryDocs.length + children.reduce((sum, child) => sum + documentsIn(child).length, 0);
    return <div className={`doc-group doc-group-level-${level} ${collapsed ? "collapsed" : ""}`} key={category.id}><button className="doc-group-toggle" aria-expanded={!collapsed} onClick={() => toggleCategory(category.id)}><ActionIcon name="folder" size={13} /><span>{category.name}</span><small>{total}</small><ActionIcon name="expand" size={14} /></button>{!collapsed && <>{categoryDocs.map((item) => <button className={item.id === record?.id ? "active" : ""} title={item.title} onClick={() => onSelect(item.id)} key={item.id}><i className="document-tree-icon" style={{ color: item.treeIconColor || "#53617b" }}><WorkspaceIcon name={item.treeIcon || "docs"} size={14} /></i><span>{item.title}</span>{item.status && item.status !== "published" && <em className={`nav-status ${item.status}`}>{item.status === "draft" ? "草稿" : "归档"}</em>}</button>)}{children.map((child) => navigationCategory(child, 1))}{!categoryDocs.length && !children.length && <small className="doc-group-empty">暂无文件</small>}</>}</div>;
  }
  ...
  return <section className="sop-page docs-page">
    ...
    <div className={`docs-layout sop-docs-layout ${navCollapsed ? "collapsed" : ""}`}><aside title="当前产品 SOP 目录"><button className="docs-nav-toggle" type="button" aria-expanded={!navCollapsed} aria-label={navCollapsed ? "展开目录" : "收起目录"} title={navCollapsed ? "展开目录" : "收起目录"} onClick={() => setNavCollapsed((current) => !current)}><ActionIcon name={navCollapsed ? "expandPanel" : "collapsePanel"} /><span>{navCollapsed ? "展开目录" : "收起目录"}</span></button>{!navCollapsed && topCategories.map((category) => navigationCategory(category))}</aside>
...
function PermissionsDialog({ profile, data, onClose, embedded = false }: { profile: UserRecord; data: WorkspaceState; onClose?: () => void; embedded?: boolean }) {
  ...
  const [newGeneralPermission, setNewGeneralPermission] = useState<"view" | "edit">("view");
  const [newGrants, setNewGrants] = useState<ScopeGrant[]>([]);
  const [busy, setBusy] = useState(false);
  ...
  async function addMember(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/users", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: form.get("username"), password: form.get("password"), name: form.get("name"), role: newRole, generalPermission: newGeneralPermission, grants: newGrants }) });
    const body = await response.json(); setBusy(false); if (!response.ok) { setError(body.error || "账号创建失败"); return; }
    setAdding(false); setNewRole("viewer"); setNewGeneralPermission("view"); setNewGrants([]); await load();
  }
  ...
  const editors = rows.filter((user) => user.role === "admin" || user.generalPermission === "edit" || user.grants?.some((grant) => grant.permission === "edit")).length;
  const editingUser = rows.find((user) => user.id === editingId) || null;
  const editorRole = adding ? newRole : editingUser?.role || "viewer";
  function closeEditor() {
    setAdding(false); setEditingId(null); setError("");
    setNewRole("viewer"); setNewGeneralPermission("view"); setNewGrants([]);
    if (editingUser) void load();
  }
  const memberEditor = (adding || editingUser) && <div className="modal-backdrop permission-editor-backdrop">
    <form className="modal member-permission-modal" onSubmit={adding ? addMember : (event) => { event.preventDefault(); if (editingUser) void updateMember(editingUser); }}>
      <header><div><span>MEMBER ACCESS</span><h2>{adding ? "添加成员" : `修改权限 · ${editingUser?.name}`}</h2><p>先设置账号身份，再按全部内容、品牌和产品线分配查看或编辑权限。</p></div><button className="icon-button" type="button" title="关闭" onClick={closeEditor}><ActionIcon name="close" /></button></header>
      {error && <div className="form-error">{error}</div>}
      <div className="member-account-fields">
        {adding ? <><label>显示名称<input name="name" required placeholder="例如：知识管理员" /></label><label>登录账号<input className="ascii-credential-input" name="username" type="text" required minLength={3} maxLength={30} pattern={USERNAME_PATTERN} inputMode="url" lang="en" autoCapitalize="none" autoCorrect="off" spellCheck={false} placeholder="knowledge-admin" title="仅可使用英文字母、数字、点、下划线或短横线" onInput={(event) => { event.currentTarget.value = sanitizeUsername(event.currentTarget.value); }} /></label><label>初始密码<input className="ascii-credential-input" name="password" required minLength={8} type="password" pattern={ASCII_PASSWORD_PATTERN} inputMode="url" lang="en" autoCapitalize="none" autoCorrect="off" spellCheck={false} placeholder="至少 8 位半角字符" title="仅可使用半角英文、数字和符号" onInput={(event) => { event.currentTarget.value = sanitizeAsciiPassword(event.currentTarget.value); }} /></label></> : <><label>显示名称<input value={editingUser?.name || ""} readOnly /></label><label>登录账号<input value={editingUser?.email.split("@")[0] || ""} readOnly /></label></>}
        <label>身份<select value={editorRole === "admin" ? "admin" : "viewer"} disabled={!adding && editingUser?.id === profile.id} onChange={(event) => adding ? setNewRole(event.target.value as Role) : editingUser && change(editingUser.id, { role: event.target.value as Role })}><option value="viewer">普通成员</option><option value="admin">管理员</option></select></label>
      </div>
      {editorRole === "admin" ? <div className="admin-permission-note"><b>管理员拥有全部管理、查看和编辑权限，无需逐项配置。</b></div> : <ScopePermissionTable data={data} value={adding ? newGrants : editingUser?.grants || []} disabled={!adding && editingUser?.id === profile.id} onChange={(grants) => adding ? setNewGrants(grants) : editingUser && change(editingUser.id, { grants })} generalPermission={adding ? newGeneralPermission : editingUser?.generalPermission || "view"} onGeneralPermissionChange={(generalPermission) => adding ? setNewGeneralPermission(generalPermission) : editingUser && change(editingUser.id, { generalPermission })} />}
      <footer className="member-permission-actions">
        {!adding && editingUser && editingUser.id !== profile.id && <div className="member-lifecycle-actions"><button type="button" disabled={busy} onClick={() => accountAction(editingUser, "reset-password")}>重置密码</button><button type="button" className={editingUser.banned ? "enable" : "danger-action"} disabled={busy} onClick={() => accountAction(editingUser, editingUser.banned ? "unban" : "ban")}>{editingUser.banned ? "启用账号" : "停用账号"}</button><button type="button" className="danger-action" disabled={busy} onClick={() => deleteMember(editingUser)}><ActionIcon name="delete" />删除账号</button></div>}
        <div><button type="button" onClick={closeEditor}>取消</button><button className="primary" disabled={busy}><ActionIcon name="save" />{busy ? "正在保存…" : adding ? "创建账号" : "保存权限"}</button></div>
      </footer>
    </form>
  </div>;
  const panel = <div className={`permissions-modal ${embedded ? "embedded" : "modal"}`}>
    {!embedded && <header><div><span>ACCESS CONTROL</span><h2>账号与权限</h2><p>管理员权限不变；普通成员可按品牌和产品线分别设置查看或编辑权限。</p></div><button className="icon-button" title="关闭" onClick={onClose}><ActionIcon name="close" /></button></header>}
    {error && <div className="form-error">{error}</div>}
    <div className="permission-summary"><div><b>{rows.length}</b><span>成员</span></div><div><b>{editors}</b><span>有编辑授权</span></div><div><b>{rows.filter((user) => user.role === "admin").length}</b><span>管理员</span></div><button onClick={() => { setError(""); setAdding(true); }}><ActionIcon name="add" />添加成员</button></div>
    <div className="permission-table"><div className="table-head"><span>成员</span><span>身份</span><span>权限摘要</span><span>操作</span></div>{rows.map((user) => {
      return <div className="table-row" key={user.id}>
        <span><i>{initials(user.name)}</i><b>{user.name}<small>{user.email.split("@")[0]} · {user.banned ? "已停用" : "正常"}</small></b></span>
        <span><b>{user.role === "admin" ? "管理员" : "普通成员"}</b></span>
        <span>{user.role === "admin" ? <small className="scope-summary">全部管理权限</small> : <small className="scope-summary">{describePermissionGrants(data, user)}</small>}</span>
        <span className="row-actions"><button disabled={busy} onClick={() => { setError(""); setEditingId(user.id); }}><ActionIcon name="edit" />修改</button></span>
      </div>;
    })}</div>
    <footer><small>编辑权限自动包含查看权限；品牌继承“全部品牌与产品”，产品线继承所属品牌。</small>{!embedded && <button className="primary" onClick={onClose}>完成</button>}</footer>
  </div>;
  return embedded ? <>{panel}{memberEditor}</> : <div className="modal-backdrop">{panel}{memberEditor}</div>;
}

function describePermissionGrants(data: WorkspaceState, user: UserRecord) {
  const all = user.grants?.find((grant) => grant.scopeType === "all" && grant.scopeId === "*");
  const parts = [`通用内容：${user.generalPermission === "edit" ? "可编辑" : "仅查看"}`];
  if (all) parts.push(`全部产品：${all.permission === "edit" ? "可编辑" : all.permission === "view" ? "仅查看" : "无权限"}`);
  const editCount = data.brands.flatMap((brand) => (data.productsByBrand[brand] || []).map((product) => resolveWorkspacePermission(data, { ...user, scopes: user.scopes || [] }, brand, product))).filter((permission) => permission === "edit").length;
  const viewCount = data.brands.flatMap((brand) => (data.productsByBrand[brand] || []).map((product) => resolveWorkspacePermission(data, { ...user, scopes: user.scopes || [] }, brand, product))).filter((permission) => permission === "view").length;
  parts.push(`${editCount} 个产品线可编辑`, `${viewCount} 个产品线仅查看`);
  return parts.join(" · ");
}

function ScopePermissionTable({ data, value, onChange, generalPermission, onGeneralPermissionChange, disabled = false }: { data: WorkspaceState; value: ScopeGrant[]; onChange: (value: ScopeGrant[]) => void; generalPermission: "view" | "edit"; onGeneralPermissionChange: (value: "view" | "edit") => void; disabled?: boolean }) {
  const permissionFor = (scopeType: ScopeGrant["scopeType"], scopeId: string, brand?: string, product?: string) => {
    if (scopeType === "all") return value.find((grant) => grant.scopeType === "all" && grant.scopeId === "*")?.permission || "none";
    return resolveWorkspacePermission(data, { id: "permission-preview", email: "", name: "", role: "viewer", scopes: [], grants: value }, brand || "", product);
  };
  function explicit(scopeType: ScopeGrant["scopeType"], scopeId: string) { return value.some((grant) => grant.scopeType === scopeType && grant.scopeId === scopeId); }
  function update(scopeType: ScopeGrant["scopeType"], scopeId: string, permission: ScopeGrant["permission"] | "inherit") {
    const remaining = value.filter((grant) => grant.scopeType !== scopeType || grant.scopeId !== scopeId);
    onChange(permission === "inherit" ? remaining : [...remaining, { scopeType, scopeId, permission }]);
  }
  function permissionCells(label: string, scopeType: ScopeGrant["scopeType"], scopeId: string, permission: ScopeGrant["permission"], canInherit: boolean) {
    const canView = permission === "view" || permission === "edit", canEdit = permission === "edit";
    return <><span className="permission-name">{label}{canInherit && explicit(scopeType, scopeId) && <button type="button" disabled={disabled} onClick={() => update(scopeType, scopeId, "inherit")}>恢复继承</button>}</span><label className="permission-check"><input type="checkbox" aria-label={`${label} 查看`} checked={canView} disabled={disabled} onChange={(event) => update(scopeType, scopeId, event.target.checked ? (canEdit ? "edit" : "view") : "none")} /></label><label className="permission-check"><input type="checkbox" aria-label={`${label} 编辑`} checked={canEdit} disabled={disabled} onChange={(event) => update(scopeType, scopeId, event.target.checked ? "edit" : (canView ? "view" : "none"))} /></label></>;
  }
  const allPermission = permissionFor("all", "*");
  return <div className="scope-permission-editor">
    <section className="permission-global-panel">
      <div className="permission-global-title"><div><b>全部内容权限</b><small>通用内容与全部品牌产品的默认权限</small></div><span><i>查看</i><i>编辑</i></span></div>
      <div className="permission-global-scopes">
        <div className="permission-global-scope"><b>通用内容</b><label><input type="checkbox" aria-label="通用内容 查看" checked disabled />查看</label><label><input type="checkbox" aria-label="通用内容 编辑" checked={generalPermission === "edit"} disabled={disabled} onChange={(event) => onGeneralPermissionChange(event.target.checked ? "edit" : "view")} />编辑</label></div>
        <div className="permission-global-scope"><b>全部品牌与产品</b><label><input type="checkbox" aria-label="全部品牌与产品 查看" checked={allPermission !== "none"} disabled={disabled} onChange={(event) => update("all", "*", event.target.checked ? (allPermission === "edit" ? "edit" : "view") : "none")} />查看</label><label><input type="checkbox" aria-label="全部品牌与产品 编辑" checked={allPermission === "edit"} disabled={disabled} onChange={(event) => update("all", "*", event.target.checked ? "edit" : (allPermission === "none" ? "none" : "view"))} />编辑</label></div>
      </div>
    </section>
    <div className="permission-section-heading"><div><b>品牌与产品线权限</b><small>每个品牌单独成组；产品线默认继承所属品牌</small></div><span>编辑权限自动包含查看权限</span></div>
    <div className="brand-permission-grid">{data.brands.map((brand) => <section className="scope-permission-brand" key={data.brandIds[brand]}>
      <div className="scope-permission-head"><span>品牌 / 产品线</span><b>查看</b><b>编辑</b></div>
      <div className="scope-permission-row brand-row">{permissionCells(brand, "brand", data.brandIds[brand], permissionFor("brand", data.brandIds[brand], brand), true)}</div>
      {(data.productsByBrand[brand] || []).map((product) => { const id = data.productIds[`${brand}/${product}`]; return <div className="scope-permission-row product-row" key={id}>{permissionCells(product, "product", id, permissionFor("product", id, brand, product), true)}</div>; })}
    </section>)}</div>
  </div>;
}
